...
Conventions for every project in this workspace, and the capability rules for the agent that
works in them. Both, because they ended up in one file when the conventions moved here: the
logbook removed its `CLAUDE.md` and this is where it went.

Claude Code reads `.claude/CLAUDE.md` at the workspace root, which is why the move was worth
making. The logbook sits two levels down at `0-infrastructure/logbook`, so a reader inside a
sibling checkout never got this by proximity and it was loaded because it was named. Here it
is loaded because of where it is, and nothing has to remember to name it.

A `linkfile` in `default.xml` symlinked the logbook's copy to the workspace root for exactly
that reason, and it was removed: a `CLAUDE.md` at the root of a repo workspace is not a file
the workspace owns, and putting one there made every tool that reads upward from a checkout
pick up a document that belongs to one project among fifty-three. This repository is the
opposite arrangement -- an ordinary project, checked out rather than linked, at the path the
tool reads. A symlink is invisible to `repo status` and to every gate here; a repository is
not.

The conventions below say how work is done across every repository in the workspace. The
capability sections at the end say what an agent may do without asking, which is a different
kind of rule and the reason this repository exists at all. A repository's own `CLAUDE.md`
still says what that repository is.
...
checkable. One of them is already not what a reader would guess — `contract-http3-queue`
defaults to `master` unlike every other `lean-*`. Stating all of them makes that an ordinary
line rather than a discovery, and makes the next one visible in a diff.
...
- A **mirror** is exempt. Its README belongs to its upstream, and editing it forks a
  ...
  evidence: `datasource-foundationdb` and `transport-xr-grid` carry GitHub's fork flag. Owning
  the Windows builds of FoundationDB is not owning its code, so its README is not ours to cut.
...
`name="dot-claude" path=".claude"`: the path is fixed by Claude Code, which reads that and
nothing else, so the name gave way instead.
