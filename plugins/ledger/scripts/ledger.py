...
report with a txn-set checksum, so "the books changed" is a hash comparison.
  ...
  ledger.py where            the ledger directory this run resolved to

This is a Claude Code plugin's script and the books are not in it. `/ledger:build` and the
three commands beside it are capability -- what an agent may do without being asked twice --
and capability lives in the agent configuration with the rest of it. A spent second is a
record of this workspace and stays in the repository that keeps records, tracked, reviewed
and gated there. `_ledger_dir()` is the join between the two, and it is one function rather
than a constant because the two are no longer in the same checkout.
"""
import argparse
import collections
import datetime
...
# Where this script is, which since it became a plugin is no longer where the books are.
# `CLAUDE_PLUGIN_ROOT` is what Claude Code exports; the fallback keeps it runnable by hand,
# the way hrr-memory's scripts are, so `python3 scripts/ledger.py report` still works.
PLUGIN_ROOT = pathlib.Path(os.environ.get("CLAUDE_PLUGIN_ROOT")
                           or pathlib.Path(__file__).resolve().parents[1]).resolve()


def _ledger_dir():
    """The directory holding spent.toml, planned.toml and the two book directories.

    `FABRIC_LEDGER` names it outright when it is set, for the same reason `FABRIC_MANIFEST`
    names the manifest: a bare clone has no workspace around it to search, and CI is that
    clone. Otherwise the logbook checkout in a synced workspace, found by climbing from this
    plugin for the directory `repo sync` puts every project under; and then, for a clone of
    just the logbook with nothing above it, that clone's own root.

    Not finding it raises. There is no empty default and no climb that ends in a guess: a
    ledger reader that finds no books reports 0 s spent to a gate that cannot tell that from
    a lull, which is the silent-skip failure this repository already refuses everywhere else.
    Getting the same question wrong once already cut the ledger from 326 sessions to 3, and
    that was a climb quietly finding the wrong root rather than saying it could not.
    """
    named = os.environ.get("FABRIC_LEDGER")
    if named:
        d = pathlib.Path(named).resolve()
        if not (d / "spent.toml").is_file():
            raise SystemExit(f"FABRIC_LEDGER names {d}, which holds no spent.toml")
        return d
    tried = []
    for parent in [PLUGIN_ROOT, *PLUGIN_ROOT.parents]:
        if (parent / ".repo").is_dir():
            d = parent / "0-infrastructure" / "logbook" / "ledger"
            tried.append(d)
            if (d / "spent.toml").is_file():
                return d
    for parent in [pathlib.Path.cwd().resolve(), *pathlib.Path.cwd().resolve().parents]:
        d = parent / "ledger"
        if (d / "spent.toml").is_file():
            return d
        tried.append(d)
    raise SystemExit("no ledger found: set FABRIC_LEDGER to the directory holding "
                     "spent.toml, or run this inside a workspace that has one. Looked at "
                     + ", ".join(str(t) for t in tried[:4]))


# The books' repository, which is what every path below is relative to and what
# `_workspace_root` climbs from. It is the logbook checkout, not this plugin.
LEDGER = _ledger_dir()
ROOT = LEDGER.parent
...
SPENT = LEDGER / "spent.toml"
SPENT_ACCOUNTS = LEDGER / "spent-accounts.toml"
SPENT_DIR = LEDGER / "spent"
PLANNED = LEDGER / "planned.toml"
PLANNED_ACCOUNTS = LEDGER / "planned-accounts.toml"
PLANNED_DIR = LEDGER / "planned"
...
def report(since_days, by_project=False):
    ...
    for _day, acct, secs, _who in rows:
...
def verify():
    ...
    for conf in (SPENT, PLANNED):
        ...
        try:
            ...
            print("  tackler is not installed: cargo install tackler")
            return 1
...
def _peer_writing():
    """Another `build` or `verify` already running on this machine, or None.

    Both rewrite every book under `ledger/spent/` -- verify regenerates them to compare --
    so two at once have them overwriting each other's output. `CLAUDE.md` calls the ledger
    exclusive and asks for a handshake before either; this is what catches the case where
    the handshake did not happen, and it lives in the tool rather than in the slash command
    so that a hand-run script is held to it too.

    It catches a collision, never an intention. An agent about to start a build is invisible
    to `ps`, which is why the handshake is still the thing that holds.

    `pid,args` rather than `args` so this process can be excluded by number: the pattern
    matches our own command line exactly as well as a peer's, and a guard that always fires
    on itself is a guard that gets deleted. Unix-only; where `ps` is absent this cannot tell,
    and it says nothing rather than blocking work it cannot see.
    """
    try:
    ...
    for line in out.splitlines()[1:]:
        ...
        if not pid.isdigit() or int(pid) == mine:
            continue
        if re.match(r"^\S*python[0-9.]*\s.*ledger\.py\s+(build|verify)\b", args, re.I):
            return f"{pid} {args.strip()[:110]}"
    return None


def _book_status():
    """What the run left changed in the books, as git sees it.

    Printed by `build` and `verify` because the answer to "did that do anything" is a
    property of the tracked files and not of the tool's own summary line. An empty status
    after a build means the books were already what git says, which is the ordinary outcome
    and worth saying out loud rather than leaving to be inferred from silence.
    """
    r = subprocess.run(["git", "-C", str(LEDGER), "status", "--porcelain", "--", "."],
                       capture_output=True, text=True, encoding="utf-8")
    if r.returncode != 0:
        return "  the books are not in a git checkout, so nothing compares them"
    lines = r.stdout.splitlines()
    if not lines:
        return f"  {LEDGER}: no change; the books already say what git says"
    return f"  {LEDGER}: {len(lines)} file(s) changed\n" + "\n".join("  " + l for l in lines)
...
def main():
    ...
    a = p.parse_args()
    if a.cmd in ("build", "verify"):
        peer = _peer_writing()
        if peer:
            print(f"the ledger is exclusive and one is already running: {peer}")
            return 1
    if a.cmd == "build":
        n, files = build()
